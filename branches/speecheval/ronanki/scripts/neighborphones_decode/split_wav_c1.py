...
if len(sys.argv) != 5:
        print '\n Usage: python split_wav.py <input_phoneseg_file> <nbg_key_mapper> <input_wav_file> <out_split_dir> \n'
        sys.exit(1)
	...
	if(j==len(allphones)):
		...
		if(allphones[j][3]=='b'):
			...
			if(allphones[j+1][3]=='b'):
				...
				ph_end = ((float(allphones[j][1])*10)+15)/1000;
				phlist = '('+cmuphones[allphones[j][2]]+')'
			elif(allphones[j+1][3]=='e'):
				ph_start =  (((float(allphones[j][0])-1)*10)+1)/1000;
				ph_end = ((float(allphones[j+1][1])*10)+15)/1000;
				phlist = '('+cmuphones[allphones[j][2]]+') ('+allphones[j+1][2]+')'
			else:
				ph_start =  (((float(allphones[j][0])-1)*10)+1)/1000;
				ph_end = ((float(allphones[j+2][1])*10)+15)/1000;

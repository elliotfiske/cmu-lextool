...
if len(sys.argv) != 4:
        print '\n Usage: python calc_ref_data.py <input_phonesegdir> <CMU_phonelist> <output_data_file> \n'
        sys.exit(1)
...
for i in fp.readlines():
        ...
        while True:
                ...
                if(s[0].isdigit()):
                        ...
                        allphones.append([s[3],s[2],phone_duration])
	...
	if i==len(cmuphones):
		...
		while 1:
			...
			if ph==allphones[j][0]:
				...
				res1=res1+int(allphones[j][1])
				res2=res2+int(allphones[j][2])
				count=count+1
			j=j+1
		if count==0:
			count=1
		cmuphones[i][1]=str(float(res1/count))
		mean_arr = [float(cmuphones[i][1])] * len(acs_scores)
		diff_arr=acs_scores
		for x in mean_arr:
			if diff_arr.count(x)!=0:
				diff_arr.remove(x)
		k=0
		while 1:
			if k==len(diff_arr):

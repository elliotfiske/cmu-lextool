...
def WikiDocument(out, id, title, text):
    ...
    print >> out, '<doc id="%s" url="%s">' % (id, url)#print >> out, '<doc id="%s" url="%s" title="%s">' % (id, url, title.encode('utf-8'))
...
class OutputSplitter:
    ...
    def write(self, text):
        text_len = len(text)
        if self.cur_file_size + text_len / 2 > self.max_file_size:
            self.close()
            self.out_file = self.open_next_file()
            self.cur_file_size = 0
        self.out_file.write(text)
        self.cur_file_size += text_len
...
def main():
    ...
    try:
        long_opts = ['help', 'compress', 'bytes=', 'basename=','links', 'sections', 'output=']
        opts, args = getopt.gnu_getopt(sys.argv[1:], 'cb:lo:B:s', long_opts)
